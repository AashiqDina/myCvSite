...
function squircle(currentDegrees,shapeParameter){
   let newX = Math.pow(Math.abs(Math.cos(currentDegrees)),2/shapeParameter) * Math.sign(Math.cos(currentDegrees));
   let newY = Math.pow(Math.abs(Math.sin(currentDegrees)),2/shapeParameter) * Math.sign(Math.sin(currentDegrees));
...
window.onload = async function()
{
   ...
   //  vertices = [];
   //  colours = [];
   let vertices_colours = [];
    indices = [];

    var currentDegrees = 0;
    var sideLength = 0.99;
    var degreesToAdd = 2*Math.PI/num_vertices;
    var scaleParameter = 0;
    var num_cycles = 16;

      for(let i = 0; i<=num_vertices ; i=i+2){
         // let tempX = sideLength *  Math.cos(currentDegrees) * scaleParameter;
         // let tempY = sideLength * Math.sin(currentDegrees) * scaleParameter;

         // vertices[i] = [tempX,tempY];
         let tempSquircle = squircle(currentDegrees, 5);
         vertices_colours[i] = [tempSquircle[0]*scaleParameter,tempSquircle[1]*scaleParameter];
         currentDegrees += num_cycles*degreesToAdd;
         indices[i] = i;
         vertices_colours[i+1] = rgba_wheel(currentDegrees);
         scaleParameter += 1/num_vertices;
    ...
    gl.bufferData(gl.ARRAY_BUFFER, mat_float_flat(vertices_colours), gl.STATIC_DRAW);
    ...
    gl.vertexAttribPointer(vertex_loc, 2, gl.FLOAT, false, 8, 0);
    ...
    gl.vertexAttribPointer(colour_loc, 4, gl.FLOAT, false, 16, 8);
